...
const AdminDomains = React.forwardRef<HTMLDivElement>((_, ref) => {
  ...
  const handleAddDomain = async () => {
    ...
    // Conexão com projeto é opcional - pode ser configurada depois
    ...
    try {
      ...
      const { error } = await supabase.from("custom_domains").insert({
        ...
        slug: slug.trim() || null,
        is_native: isNative,
        content_type: contentType === "none" ? null : contentType,
        content_id: contentType === "none" || contentType === "main_lp" ? null : contentId,
        is_active: true,
        user_id: user.id,
      });
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">
                      {d.content_type ? `Apontando para: ${d.content_type === 'main_lp' ? 'Landing Page Principal' : d.content_type === 'quiz' ? 'Quiz' : 'Página Gerada'}` : 'Sem conteúdo vinculado (configure depois)'}
                    </p>
                  </div>
                </div>
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="w-full">
            <p className="text-sm font-bold mb-4">Como Configurar Seu Domínio (Hospedagem Nativa)</p>
            <div className="space-y-6">
              <p className="text-xs text-muted-foreground leading-relaxed">
                Acesse o painel de controle do seu provedor de domínio (Cloudflare, Hostinger, GoDaddy, Namecheap, etc.) e adicione um dos registros DNS abaixo:
              </p>
              ...
              <div className="p-4 rounded-lg bg-background border border-border/40">
                <p className="text-[11px] font-bold text-primary uppercase mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-[9px] font-bold">1</span>
              ...
              <div className="p-4 rounded-lg bg-background border border-border/40">
                <p className="text-[11px] font-bold text-primary uppercase mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-[9px] font-bold">2</span>
                ...
                <div className="space-y-2 font-mono text-[11px]">
                  ...
                  <div className="flex items-center justify-between p-2 bg-secondary/30 rounded">
                    <span className="text-muted-foreground">Nome:</span>
                    <span className="font-bold">www (ou seu subdomínio)</span>
                  </div>
                  <div className="flex items-center justify-between p-2 bg-secondary/30 rounded">
                    <span className="text-muted-foreground">Valor (Alvo):</span>
                    <span className="font-bold text-primary">cname.coworkingellite.com</span>
                  </div>
                  <div className="flex items-center justify-between p-2 bg-secondary/30 rounded">
                    <span className="text-muted-foreground">TTL:</span>
                    <span className="font-bold">3600 (1 hora) ou Auto</span>
              ...
              <div className="p-3 rounded-lg bg-secondary/20 border border-border/30">
                ...
                <ul className="text-[10px] text-muted-foreground space-y-1">
                  <li><strong>Tipo:</strong> A (IPv4) ou CNAME (alias de domínio)</li>
                  <li><strong>Nome:</strong> @ para domínio raiz (exemplo.com), www para subdomínio (www.exemplo.com)</li>
                  <li><strong>Valor:</strong> IP do servidor (Registro A) ou destino CNAME</li>
