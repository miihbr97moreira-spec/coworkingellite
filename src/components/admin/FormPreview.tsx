...
const FormPreview = ({ form }: FormProps) => {
    ...
    if (q.logic && answer) {
      ...
      for (const rule of q.logic) {
        if (rule.condition_value && answerStr.toLowerCase() === rule.condition_value.toLowerCase()) {
          if (rule.action === "finish") { setFinished(true); return; }
  ...
  }, [q, answers, currentIdx, questions]);
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            ...
            {q.description && (
              <p className="text-base opacity-70" style={{ color: theme.textColor }}>{q.description}</p>
            )}
            ...
            {q.type === "short_text" && (
              <input
                ...
                className="w-full bg-transparent border-b-2 text-xl py-3 outline-none placeholder-opacity-40"
                style={{ color: theme.textColor, borderColor: `${theme.textColor}30` }}
                autoFocus
              />
            )}
            ...
            {q.type === "long_text" && (
              <textarea
                ...
                onChange={e => setAnswer(e.target.value)}
                rows={4}
                className="w-full bg-transparent border-b-2 text-lg py-3 outline-none resize-none placeholder-opacity-40"
                style={{ color: theme.textColor, borderColor: `${theme.textColor}30` }}
                autoFocus
              />
            )}
            ...
            {q.type === "email" && (
              <input
                ...
                value={answers[q.id] || ""}
                onChange={e => setAnswer(e.target.value)}
                className="w-full bg-transparent border-b-2 text-xl py-3 outline-none"
                style={{ color: theme.textColor, borderColor: `${theme.textColor}30` }}
                autoFocus
              />
            )}
                ...
                {(q.options || []).map((opt, i) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => { setAnswer(opt); setTimeout(goNext, 300); }}
                      className="w-full flex items-center gap-3 px-5 py-4 rounded-xl border-2 text-left transition-all"
                      style={{
                        borderColor: selected ? theme.buttonColor : `${theme.textColor}20`,
                        backgroundColor: selected ? `${theme.buttonColor}15` : "transparent",
                        color: theme.textColor,
                      }}
                    >
                      <span className="w-7 h-7 rounded-md border-2 flex items-center justify-center text-xs font-bold shrink-0" style={{ borderColor: selected ? theme.buttonColor : `${theme.textColor}40` }}>
                  ...
                  return (
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        setAnswer(selected ? current.filter((x: string) => x !== opt) : [...current, opt]);
                      }}
                      className="w-full flex items-center gap-3 px-5 py-4 rounded-xl border-2 text-left transition-all"
                      style={{
                        borderColor: selected ? theme.buttonColor : `${theme.textColor}20`,
              ...
              <div className="flex gap-2">
                {Array.from({ length: q.max || 5 }, (_, i) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={() => { setAnswer(i + 1); setTimeout(goNext, 400); }}
                  >
                ...
                {Array.from({ length: (q.max || 10) - (q.min || 1) + 1 }, (_, i) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => { setAnswer(val); setTimeout(goNext, 300); }}
            ...
            {(q.type === "welcome" || q.type === "statement") && null}

            {/* Navigation */}
            <div className="flex items-center gap-3 pt-4">
              {q.type !== "multiple_choice" && q.type !== "rating" && q.type !== "opinion_scale" && q.type !== "yes_no" && (
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={goNext}
              ...
              {q.type !== "welcome" && q.type !== "statement" && (
                <span className="text-xs opacity-40 ml-auto" style={{ color: theme.textColor }}>
